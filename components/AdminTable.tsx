...
const AdminTable: React.FC<AdminTableProps> = ({ userData, emailList }) => {
  ...
  const filteredData = useMemo(() => {
    return transposedData.filter((row) => {
      ...
      const matchesValue = filterValue ? Object.values(row).some((val) => typeof val === "string" && val.includes(filterValue)) : true;
  ...
  const convertToPDF = async () => {
    ...
    try {
      ...
      html2pdf()
        ...
        .toPdf()
        .get("pdf")
        .then((pdf: { internal: { getNumberOfPages: () => any; pageSize: { getWidth: () => number; getHeight: () => number; }; }; setPage: (arg0: number) => void; setFontSize: (arg0: number) => void; text: (arg0: string, arg1: number, arg2: number) => void; }) => {
          const totalPages = pdf.internal.getNumberOfPages();
          for (let i = 1; i <= totalPages; i++) {
            pdf.setPage(i);
            pdf.setFontSize(10);
  ...
  return (
    <div>
      <div className="grid lg:flex lg:justify-between py-4 space-y-2">
        ...
        <div className="flex gap-4">
          <button className="text-xs lg:text-lg p-2 text-white border-2 bg-teal-700 rounded-md" onClick={convertToCSV}>
            Download CSV
          </button>
          <button className="text-xs lg:text-lg p-2 text-white border-2 bg-teal-700 rounded-md" onClick={convertToPDF}>
            Download PDF
          </button>
        </div>
      </div>
          ...
          <tbody>
            {filteredData.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-gray-100">
                ...
                {monthDays.map((day) => <td key={day} className="border border-gray-300 px-6 py-2 text-center">{row[day]}</td>)}
              </tr>
            ))}
          </tbody>
        </table>
