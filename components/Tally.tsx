...
interface UserRecords {
  [date: string]: {
    [key: string]: string | number; // Daily data for each key
  };
}

interface LabelMap {
  [key: string]: string; // Mapping of keys to labels
}
...
// Tally Component
const Tally: React.FC<TallyProps> = ({ userData = { records: {} }, email, title }) => {
  // Function to determine background color based on percentage
  ...
  const aggregateUserData = (userData: UserData, email: string) => {
    ...
    Object.values(userRecords).forEach((dailyData) => {
      Object.entries(dailyData).forEach(([key, value]) => {
        if (aggregatedData[key] !== undefined) {
          aggregatedData[key] += parseInt(value as string, 10);
        }
      });
    });
