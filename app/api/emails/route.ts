...
export async function POST(request: Request) {
  const { email, name, leaveType, reason, leaveDates, action } = await request.json();
  ...
  if (action === 'leaveApplication') {
    ...
    return new Response(JSON.stringify({ message: 'Leave application email sent successfully!' }), {
      status: 200,
    ...
    return new Response(JSON.stringify({ message: 'Leave approval email sent successfully!' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
    ...
    return new Response(JSON.stringify({ message: 'Leave rejection email sent successfully!' }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } else {
